...
const SpotButton: React.FC<SpotButtonProps> = ({
  onSpot
}) => {
  ...
  return <div className="space-y-4">
      ...
      <div className="relative">
        <Button onClick={handleSpot} disabled={isSpotting} className={`w-full py-6 text-lg font-semibold transition-all duration-300 ${isSpotting ? 'bg-green-600 hover:bg-green-600' : 'bg-gradient-to-r from-slate-800 to-slate-600 hover:from-slate-700 hover:to-slate-500'}`}>
          {isSpotting ? <span className="flex items-center space-x-2">
              <Check className="w-5 h-5" />
              <span>Treasure Spotted! 🪶</span>
            </span> : <span className="flex items-center space-x-2">
              <Eye className="w-5 h-5" />
              <span>Save to MyMagPye</span>
              <Sparkles className="w-4 h-4" />
            </span>}
        </Button>
        
        {isSpotting && <div className="absolute inset-0 bg-green-500 rounded-md opacity-20 animate-pulse" />}
      </div>
